...
def get_model(name, **kwargs):
    ...
    if name == 'baseline':
        ...
        center_pixel = True
        model = HamidaEtAl(n_bands, patch_size=patch_size)
        ...
        kwargs.setdefault('scheduler', optim.lr_scheduler.MultiStepLR(
            optimizer, milestones=[50, 150], gamma=0.1))
...
class Baseline(nn.Module):
    ...
    def __init__(self, input_channels,dropout=False):
        ...
        nodes = 50
        self.fc1 = nn.Linear(input_channels, nodes)
...
class HamidaEtAl(nn.Module):
    ...
    def __init__(self, input_channels, patch_size=5, dilation=1):
        ...
        self.fc = nn.Linear(self.features_size,1) #n_classes)
...
def train(net, optimizer, criterion, data_loader, epoch,
          exp_name, cross_dict=None,
          scheduler=None, display_iter=100, device=torch.device('cpu'),
          display=None,
    ...
    for e in tqdm(range(1, epoch + 1), desc="Training the network"):
        ...
        for batch_idx, (data, target) in tqdm(enumerate(data_loader),
            ...
            if supervision == 'full':
                ...
                raise ValueError("supervision mode \"{}\" is unknown.".format(
                    supervision))
            loss.backward()
            optimizer.step()
        ...
        if val_loader is not None:
            val_acc = val(net, val_loader, device=device,
                          supervision=supervision, old_val=val_acc)
        ...
        if e == epoch:
            _ = save_model(net, camel_to_snake(str(net.__class__.__name__)),
                           data_loader.dataset.name, exp_name, cross_dict,
                           epoch=e, metric=abs(metric))
        is_best = (best_val_acc < val_acc)
        if is_best:
            best_val_acc = val_acc
            best_path = save_model(net, camel_to_snake(
                str(net.__class__.__name__)), data_loader.dataset.name,
                                    exp_name, cross_dict,
                                    is_best=True, epoch=e, metric=abs(metric))
    return best_path
    ...
    if isinstance(model, torch.nn.Module):
        if not(filename):
            filename = str(datetime.datetime.now()) + \
                    "_epoch{epoch}_{metric:.2f}".format(**kwargs)

        tqdm.write("Saving current best neural network weights in "
                   "{}".format(filename))
...
def val(net, data_loader, device='cpu', supervision='full', old_val=0):
        ...
        with torch.no_grad():
            ...
            for out, pred in zip(output.view(-1), target.view(-1)):
                if out.item() in ignored_labels:
                    ...
                    print("You are thereeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee")
                    print(type(pred.tolist()))
                    print(type(out.tolist()))
                    l =  mean_absolute_error([pred.tolist()],[out.tolist()])
                    print(type(accuracy))
                    print((l))
                    accuracy += l
                    total += 1
