...
DATASETS_CONFIG = {
        ...
        'IndianPines': {
            'urls': ['http://www.ehu.eus/ccwintco/uploads/6/67/'
        ...
        'Botswana': {
            'urls': ['http://www.ehu.es/ccwintco/uploads/7/72/Botswana.mat',
                     'http://www.ehu.es/ccwintco/uploads/5/58/'
                     'Botswana_gt.mat'],
            'img': 'Botswana.mat',
            'gt': 'Botswana_gt.mat'
            }
...
def get_dataset(dataset_name, target_folder="./", datasets=DATASETS_CONFIG):
    ...
    ./datasets (optional): datt configuration dictionary, defaults to prebuilt
    one.
    Returns:
    ...
    if dataset_name == 'PaviaC':
        ...
        # Custom dataset
        img, gt, rgb_bands, ignored_labels, \
             = CUSTOM_DATASETS_CONFIG[dataset_name]['loader'](
                folder, DATASETS_CONFIG)

    # Filter NaN out
    nan_mask = np.isnan(img.sum(axis=-1))
    if np.count_nonzero(nan_mask) > 0:
        print("Warning: NaN have been found in the data. It is preferable to\
        remove them beforehand. Learning on NaN data is disabled.")
    img[nan_mask] = 0
    gt[nan_mask] = 0
    ...
    default_dict = {
        'folder': folder,
        'cube_shape': list(img.shape)[:2],
        'img_shape': list(img.shape)[:2],
        'img_name': [dataset_name],
        'fused_class': None}
    DATASETS_CONFIG[dataset_name].setdefault('info_data', default_dict)

    img = np.asarray(img, dtype='float32')
    return img, gt, ignored_labels, rgb_bands, \
        DATASETS_CONFIG
...
class HyperX(torch.utils.data.Dataset):
    ...
    def __init__(self, data, gt, **hyperparams):
        ...
        self.indices = np.array([(x, y) for x, y in zip(x_pos, y_pos) if x > p
                                 and x < data.shape[0] - p
                                 and y > p
                                 and y < data.shape[1] - p])
    ...
    @staticmethod
    def radiation_noise(data, alpha_range=(0.8, 1.2), beta=1/15):
    ...
    def mixture_noise(self, data, label, beta=1/25):
        ...
        return (alpha1 * data + alpha2 * data2) / (alpha1 + alpha2) + \
            beta * noise
